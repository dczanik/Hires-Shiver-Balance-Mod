		...
		BOOLEAN DrawBluePrint)
{
	...
	else if (RepairSlot && !(RESOLUTION_FACTOR == 2 && DrawBluePrint))
	{
		r.corner = Side.origin;
		r.corner.y += RES_CASE(0,0,8);
		r.extent.height = (8 << RESOLUTION_FACTOR) + RES_CASE(0,0,26); // JMS_GFX
		r.extent.width = ship_piece_offset_scaled;
		r.extent.height = (1 << RESOLUTION_FACTOR);
		OldColor = SetContextForeGroundColor (BLACK_COLOR);
		DrawFilledRectangle (&r);
		r.corner.y += ((23 - 1) << RESOLUTION_FACTOR) + RES_CASE(0,0,30); // JMS_GFX
		if (which_slot == 0 && RESOLUTION_FACTOR !=0)
			...
			r.extent.width -= RES_CASE(0,3,6); // JMS_GFX
		DrawFilledRectangle (&r);

		r.extent.width = RES_CASE(1,6,12);
		r.extent.height = (8 << RESOLUTION_FACTOR) + RES_CASE(0,0,26); // JMS_GFX
		...
		if (RepairSlot == 2)
		{
			...
			if (RESOLUTION_FACTOR == 2)
			{
				...
				{
					r.extent.height -= RES_CASE(0,6,12); // JMS_GFX
			...
			if (RESOLUTION_FACTOR == 2)
			{
				...
				if (which_slot == NUM_MODULE_SLOTS - 1 && RESOLUTION_FACTOR !=0)
				{
					...
					r.extent.height -= RES_CASE(0,18,36); // JMS_GFX
	...
	if (DrawBluePrint)
	{
		if (RepairSlot && !(RESOLUTION_FACTOR == 2 && DrawBluePrint))
	...
	else
	{
		...
		else if (RepairSlot)
		{
			...
			DrawFilledRectangle (&r);
			r.corner.y += ((32 - 1) << RESOLUTION_FACTOR) + RES_CASE(0,0,16);  // JMS_GFX
			DrawFilledRectangle (&r);

			r.extent.width = (1 << RESOLUTION_FACTOR) + RESOLUTION_FACTOR;
			r.extent.height = (12 << RESOLUTION_FACTOR) + RES_CASE(0,0,17); // JMS_GFX
