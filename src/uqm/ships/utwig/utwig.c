...
initialize_lance (ELEMENT *ShipPtr, HELEMENT WeaponArray[])
{
	...
	{
		COUNT angle;
		SIZE sin0, cos0, sin1sin0, cos1sin0, cos1cos0, sin1cos0 = 0;
		SIZE dx, dy;
		ELEMENT *WeaponPtr;
        ...
        if (WeaponArray[(i << 1)] = initialize_missile (&MissileBlock))
		{
			...
			UnlockElement (WeaponArray[(i << 1)]);
		}

		// Mirror image of first projectile.
		cos0 = -cos1cos0 - sin1sin0;
		sin0 = -sin1cos0 + cos1sin0;
		MissileBlock.cx = ShipPtr->next.location.x + cos0;
		MissileBlock.cy = ShipPtr->next.location.y + sin0;
        ...
        if (WeaponArray[(i << 1) + 1] = initialize_missile (&MissileBlock))
		{
			...
			UnlockElement (WeaponArray[(i << 1) + 1]);
		}
	}
		...
		ELEMENT *ElementPtr1, POINT *pPt1)
{
	...
	// Androsynth blazer will now charge up Utwig's energy.
	if (ElementPtr0->life_span > NORMAL_LIFE
		&& ((StarShipPtr && StarShipPtr->SpeciesID == ANDROSYNTH_ID)
		&& ElementPtr1->current.image.farray == StarShipPtr->RaceDescPtr->ship_data.special))
	{
		ElementPtr0->life_span += 3;
	}
