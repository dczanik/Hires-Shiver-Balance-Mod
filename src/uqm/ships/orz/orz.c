...
marine_preprocess (ELEMENT *ElementPtr)
{
	...
	else
	{
				 ...
				 && !(ElementPtr->thrust_wait & (SHIP_IN_GRAVITY_WELL >> 6))))
		{
			...
			StarShipPtr->RaceDescPtr->characteristics.thrust_increment = MARINE_THRUST_INCREMENT;
			StarShipPtr->RaceDescPtr->characteristics.max_thrust = MARINE_MAX_THRUST;
				...
				&& (StarShipPtr->cur_status_flags & DOWN))
		{
			...
			for (hShip = GetHeadElement (); hShip != 0; hShip = hNextShip)
			{
						...
						& (GOOD_GUY | BAD_GUY)) != (GOOD_GUY | BAD_GUY)))
				{
					...
					Them->hTarget = 0;
					Them->current.image.frame =
						Them->next.image.frame =
								SetAbsFrameIndex (
								StarShipPtr->RaceDescPtr->ship_data.special[0], 21);
				}
				UnlockElement(hShip);
